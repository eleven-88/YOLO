...
layer make_oriented_yolo_layer(int batch, int w, int h, int n, int total, int *mask, int classes, int max_boxes)
{
    ...
    l.h = h;
    l.w = w;
    l.c = n*(classes + 4 + 5 + 1);  // (x,y,w,h,a1,a2,a3,a4,r)
    l.out_w = l.w;
    l.out_h = l.h;
    l.out_c = l.c;
    l.classes = classes;
    ...
    l.outputs = h*w*n*(classes + 4 + 5 + 1);
    l.inputs = l.outputs;
    l.max_boxes = max_boxes;
    l.truths = l.max_boxes*(4 + 5 + 1);    // 90*(9 + 1);
    ...
    l.forward = forward_oriented_yolo_layer;
    l.backward = backward_oriented_yolo_layer;
#ifdef GPU
    l.forward_gpu = forward_oriented_yolo_layer_gpu;
    l.backward_gpu = backward_oriented_yolo_layer_gpu;
...
void resize_oriented_yolo_layer(layer *l, int w, int h)
{
    ...
    l->outputs = h*w*l->n*(l->classes + 4 + 5 + 1);
    l->inputs = l->outputs;
...
//计算第(i, j)个cell第n个bbox在当前特征图上的相对位置[x,y],在网络输入图片上的相对宽度,高度[w,h]
box get_oriented_yolo_box(float *x, float *biases, int n, int index, int i, int j, int lw, int lh, int w, int h, int stride)
{
    box b;
...
int compare_oriented_yolo_class(float *output, int classes, int class_index, int stride, float objectness, int class_id, float conf_thresh)
{
    int j;
    for (j = 0; j < classes; ++j) {
...
float backward_smooth_l1_cpu(float pred, float truth, float sigma_squared)
{
    float delta;
    float diff = truth - pred;    // -dx
    float abs_val = fabs(diff);
    if(abs_val < 1.0 / sigma_squared) {
        delta = sigma_squared * diff;
    }
    else {
        delta = (diff > 0) ? 1 : -1;
    }
...
float forward_smooth_l1_cpu(float pred, float truth, float sigma_squared)
{
    float loss;
    float diff = pred - truth;
    float abs_val = fabs(diff);
    if(abs_val < 1.0 / sigma_squared) {
        loss = 0.5 * sigma_squared * diff * diff;
    }
    else {
        loss = abs_val - 0.5 / sigma_squared;
    }
    return loss;
}

float backward_sigmoid_smooth_l1_cpu(float pred, float truth, float sigma_squared)
{
    ...
    float delta;
    float diff = truth - pred;    // -dx
    float sigmoid_dx = pred * (1 - pred);
    float abs_val = fabs(diff);
    if(abs_val < 1.0 / sigma_squared) {
        delta = sigma_squared * diff * sigmoid_dx;
    }
    else {
        delta = (diff > 0) ? sigmoid_dx : -sigmoid_dx;
    }
    return delta;
}

ious delta_oriented_yolo_box(box truth, float *x, float *biases, int n, int index, int i, int j, int lw, int lh, int w, int h, float *delta, float scale, int stride, float iou_normalizer, IOU_LOSS iou_loss, int accumulate, float max_delta, float sigma_squared)
{
    ious all_ious = { 0 };
    // i - step in layer width
    // j - step in layer height
    //  Returns a box in absolute coordinates
    box pred = get_oriented_yolo_box(x, biases, n, index, i, j, lw, lh, w, h, stride);
    ...
    if (iou_loss == MSE)    // old loss
    {
        ...
        // accumulate delta
        delta[index + 0 * stride] += scale * backward_sigmoid_smooth_l1_cpu(x[index + 0 * stride], tx, sigma_squared) * iou_normalizer;
        delta[index + 1 * stride] += scale * backward_sigmoid_smooth_l1_cpu(x[index + 1 * stride], ty, sigma_squared) * iou_normalizer;
        delta[index + 2 * stride] += scale * backward_smooth_l1_cpu(x[index + 2 * stride], tw, sigma_squared) * iou_normalizer;
        delta[index + 3 * stride] += scale * backward_smooth_l1_cpu(x[index + 3 * stride], th, sigma_squared) * iou_normalizer;
    }
    else {
...
void averages_oriented_yolo_deltas(int class_index, int box_index, int stride, int classes, float *delta)
{
...
void delta_oriented_yolo_class(float *output, float *delta, int index, int class_id, int classes, int stride, float *avg_cat, int focal_loss, float label_smooth_eps, float *classes_multipliers)
{
    int n;
    if (delta[index + stride*class_id]){
        float y_true = 1;
        if(label_smooth_eps) y_true = y_true *  (1 - label_smooth_eps) + 0.5*label_smooth_eps;   // 二值交叉熵+sigmoid
...
float delta_yolo_orient(orient_box truth, float *output, float *delta, int index, int stride, float lamda, float sigma_squared)
{
//    printf("\n truth.a1 = %f, truth.a2 = %f, truth.a3 = %f, truth.a4 = %f, truth.r = %f ", truth.a1, truth.a2, truth.a3, truth.a4, truth.r);
//    printf("pred.a1 = %f, pred.a2 = %f, pred.a3 = %f, pred.a4 = %f, pred.r = %f \n", output[index + 0*stride], output[index + 1*stride], output[index + 2*stride], output[index + 3*stride], output[index + 4*stride]);
//    float sigma_squared = 9.0;
    delta[index + 0 * stride] += backward_sigmoid_smooth_l1_cpu(output[index + 0*stride], truth.a1, sigma_squared);
    delta[index + 1 * stride] += backward_sigmoid_smooth_l1_cpu(output[index + 1*stride], truth.a2, sigma_squared);
    delta[index + 2 * stride] += backward_sigmoid_smooth_l1_cpu(output[index + 2*stride], truth.a3, sigma_squared);
    delta[index + 3 * stride] += backward_sigmoid_smooth_l1_cpu(output[index + 3*stride], truth.a4, sigma_squared);
    delta[index + 4 * stride] += backward_sigmoid_smooth_l1_cpu(output[index + 4*stride], truth.r, sigma_squared) * lamda;

    float loss = 0;
    loss += forward_smooth_l1_cpu(output[index + 0*stride], truth.a1, sigma_squared);
    loss += forward_smooth_l1_cpu(output[index + 1*stride], truth.a1, sigma_squared);
    loss += forward_smooth_l1_cpu(output[index + 2*stride], truth.a1, sigma_squared);
    loss += forward_smooth_l1_cpu(output[index + 3*stride], truth.a1, sigma_squared);
    loss += forward_smooth_l1_cpu(output[index + 4*stride], truth.a1, sigma_squared) * lamda;

    return loss;
}

static int entry_oriented_index(layer l, int batch, int location, int entry)
{
    int n =   location / (l.w*l.h);
    int loc = location % (l.w*l.h);
    return batch*l.outputs + n*l.w*l.h*(4+5+l.classes+1) + entry*l.w*l.h + loc;
}
...
void forward_oriented_yolo_layer(const layer l, network_state state)
{
    ...
    for (b = 0; b < l.batch; ++b) {
        for (n = 0; n < l.n; ++n) {
            int index = entry_oriented_index(l, b, n*l.w*l.h, 0);
            ...
            index = entry_oriented_index(l, b, n*l.w*l.h, 4);
            activate_array(l.output + index, (5 + 1 + l.classes)*l.w*l.h, LOGISTIC);  // orient, classes and objectness
        }
    }
#endif
    ...
    *(l.cost) = 0;
    for (b = 0; b < l.batch; ++b) {
        for (j = 0; j < l.h; ++j) {
            for (i = 0; i < l.w; ++i) {
                for (n = 0; n < l.n; ++n) {
                    const int class_index = entry_oriented_index(l, b, n*l.w*l.h + j*l.w + i, 9 + 1);
                    const int obj_index = entry_oriented_index(l, b, n*l.w*l.h + j*l.w + i, 9);
                    const int box_index = entry_oriented_index(l, b, n*l.w*l.h + j*l.w + i, 0);
                    const int orient_index = entry_oriented_index(l, b, n*l.w*l.h + j*l.w + i, 4);
                    const int stride = l.w*l.h;
                    box pred = get_oriented_yolo_box(l.output, l.biases, l.mask[n], box_index, i, j, l.w, l.h, state.net.w, state.net.h, l.w*l.h);
                    float best_match_iou = 0;
                    int best_match_t = 0;
                    float best_iou = 0;
                    int best_t = 0;
                    for (t = 0; t < l.max_boxes; ++t) {
                        box truth = float_to_box_stride(state.truth + t*(4 + 5 + 1) + b*l.truths, 1);
                        int class_id = state.truth[t*(4 + 5 + 1) + b*l.truths + 4 + 5];
                        ...
                        orient_box orient_truth = float_to_orient_box_stride(state.truth + t*(4 + 5 + 1) + b*l.truths + 4, 1);
                        if (orient_truth.a1 < 0 || orient_truth.a2 < 0 || orient_truth.a3 < 0 || orient_truth.a4 < 0) {
                            printf("\n orient_truth.a1 = %f, orient_truth.a2 = %f, orient_truth.a3 = %f, orient_truth.a4 = %f \n", orient_truth.a1, orient_truth.a2, orient_truth.a3, orient_truth.a4);
                            break;
                        }

                        float objectness = l.output[obj_index];
                        if (isnan(objectness) || isinf(objectness)) l.output[obj_index] = 0;
                        int class_id_match = compare_oriented_yolo_class(l.output, l.classes, class_index, l.w*l.h, objectness, class_id, 0.25f);
                    ...
                    if (best_match_iou > l.ignore_thresh) {
                        ...
                        if (l.objectness_smooth) {
                            ...
                            int class_id = state.truth[best_match_t*(4 + 5 + 1) + b*l.truths + 4 + 5];
                    ...
                    if (best_iou > l.truth_thresh) {
                        ...
                        int class_id = state.truth[best_t*(4 + 5 + 1) + b*l.truths + 4 + 5];
                        if (l.map) class_id = l.map[class_id];
                        delta_oriented_yolo_class(l.output, l.delta, class_index, class_id, l.classes, l.w*l.h, 0, l.focal_loss, l.label_smooth_eps, l.classes_multipliers);
                        const float class_multiplier = (l.classes_multipliers) ? l.classes_multipliers[class_id] : 1.0f;
        ...
        for (t = 0; t < l.max_boxes; ++t) {
            ...
            if (mask_n >= 0) {
                ...
                int obj_index = entry_oriented_index(l, b, mask_n*l.w*l.h + j*l.w + i, 4 + 5);
                avg_obj += l.output[obj_index];
                l.delta[obj_index] = class_multiplier * l.cls_normalizer * (1 - l.output[obj_index]);

                int class_index = entry_oriented_index(l, b, mask_n*l.w*l.h + j*l.w + i, 4 + 5 + 1);
                delta_oriented_yolo_class(l.output, l.delta, class_index, class_id, l.classes, l.w*l.h, &avg_cat, l.focal_loss, l.label_smooth_eps, l.classes_multipliers);

                int orient_index = entry_oriented_index(l, b, mask_n*l.w*l.h + j*l.w + i, 4);
                orient_box orient_truth = float_to_orient_box_stride(state.truth + t*(4 + 5 + 1) + b*l.truths + 4, 1);
                tot_orient_loss += delta_yolo_orient(orient_truth, l.output, l.delta, orient_index, l.w*l.h, l.uc_normalizer, l.sigma_squared);
        ...
        for (j = 0; j < l.h; ++j) {
            for (i = 0; i < l.w; ++i) {
                for (n = 0; n < l.n; ++n) {
                    int box_index = entry_oriented_index(l, b, n*l.w*l.h + j*l.w + i, 0);
                    int orient_index = entry_oriented_index(l, b, n*l.w*l.h + j*l.w + i, 4);
                    int class_index = entry_oriented_index(l, b, n*l.w*l.h + j*l.w + i, 4 + 5 + 1);
                    const int stride = l.w*l.h;

                    averages_oriented_yolo_deltas(class_index, box_index, stride, l.classes, l.delta);
                }
            }
        }
    }
    ...
    for (b = 0; b < l.batch; ++b) {
        for (j = 0; j < l.h; ++j) {
            for (i = 0; i < l.w; ++i) {
                for (n = 0; n < l.n; ++n) {
                    int index = entry_oriented_index(l, b, n*l.w*l.h + j*l.w + i, 0);
    ...
    float* no_orient_loss_delta = (float *)calloc(l.batch * l.outputs, sizeof(float));
    memcpy(no_orient_loss_delta, l.delta, l.batch * l.outputs * sizeof(float));
    for (b = 0; b < l.batch; ++b) {
        for (j = 0; j < l.h; ++j) {
            for (i = 0; i < l.w; ++i) {
                for (n = 0; n < l.n; ++n) {
                    int index = entry_oriented_index(l, b, n*l.w*l.h + j*l.w + i, 0);
                    no_orient_loss_delta[index + 4 * stride] = 0;
                    no_orient_loss_delta[index + 5 * stride] = 0;
                    no_orient_loss_delta[index + 6 * stride] = 0;
                    no_orient_loss_delta[index + 7 * stride] = 0;
                    no_orient_loss_delta[index + 8 * stride] = 0;
                }
            }
        }
    }
    ...
    float orient_loss = loss - pow(mag_array(no_orient_loss_delta, l.outputs * l.batch), 2);
    free(no_orient_loss_delta);
    float avg_orient_loss = count > 0 ? tot_orient_loss / count : 0;

    float iou_loss = loss - classification_loss - orient_loss;
    ...
    fprintf(stderr, "v3 (%s loss, Normalizer: (iou: %.2f, cls: %.2f, orient: %.2f) Region %d Avg (IOU: %f, GIOU: %f), Class: %f, Obj: %f, No Obj: %f, .5R: %f, .75R: %f, count: %d, class_loss = %f, (iou_loss = %f avg_iou_loss = %f), (orient_loss = %f, avg_orient_loss = %f), total_loss = %f \n",
            (l.iou_loss == MSE ? "mse" : (l.iou_loss == GIOU ? "giou" : "iou")), l.iou_normalizer, l.cls_normalizer, l.uc_normalizer, state.index, tot_iou / count, tot_giou / count, avg_cat / class_count, avg_obj / count, avg_anyobj / (l.w*l.h*l.n*l.batch), recall / count, recall75 / count, count,
            classification_loss, iou_loss, avg_iou_loss, orient_loss, avg_orient_loss, loss);
}
...
void forward_oriented_yolo_layer_gpu(const layer l, network_state state)
{
    ...
    for (b = 0; b < l.batch; ++b){
        for(n = 0; n < l.n; ++n){
            ...
            index = entry_oriented_index(l, b, n*l.w*l.h, 4);
            activate_array_ongpu(l.output_gpu + index, (5+1+l.classes)*l.w*l.h, LOGISTIC); //orient, classes and objectness
        }
    }
    ...
    forward_oriented_yolo_layer(l, cpu_state);
    //forward_oriented_yolo_layer(l, state);
    cuda_push_array(l.delta_gpu, l.delta, l.batch*l.outputs);
    free(in_cpu);
    if (cpu_state.truth) free(cpu_state.truth);
}

void backward_oriented_yolo_layer_gpu(const layer l, network_state state)
{
...
int oriented_yolo_num_detections(layer l, float thresh)
{
    ...
    for(n = 0; n < l.n; ++n){
        for (i = 0; i < l.w*l.h; ++i) {
            int obj_index  = entry_oriented_index(l, 0, n*l.w*l.h + i, 4 + 5);
...
int oriented_yolo_num_detections_batch(layer l, float thresh, int batch)
{
    ...
    for (i = 0; i < l.w*l.h; ++i){
        for(n = 0; n < l.n; ++n){
            int obj_index  = entry_oriented_index(l, batch, n*l.w*l.h + i, 4 + 5);
            if(l.output[obj_index] > thresh){
                ++count;
            }
        }
...
int get_oriented_yolo_detections(layer l, int w, int h, int netw, int neth, float thresh, int *map, int relative, detection *dets, int letter)
{
    ...
    for (i = 0; i < l.w*l.h; ++i){
        ...
        int col = i % l.w;
        for(n = 0; n < l.n; ++n){
            int obj_index  = entry_oriented_index(l, 0, n*l.w*l.h + i, 9);
            float objectness = predictions[obj_index];
            if (objectness <= thresh) continue;    // incorrect behavior for Nan values

            if (objectness > thresh) {
                int box_index = entry_oriented_index(l, 0, n*l.w*l.h + i, 0);
                dets[count].bbox = get_oriented_yolo_box(predictions, l.biases, l.mask[n], box_index, col, row, l.w, l.h, netw, neth, l.w*l.h);
                dets[count].objectness = objectness;
                dets[count].classes = l.classes;

                dets[count].uc[0] = predictions[entry_oriented_index(l, 0, n*l.w*l.h + i, 4)]; // a1
                dets[count].uc[1] = predictions[entry_oriented_index(l, 0, n*l.w*l.h + i, 5)]; // a2
                dets[count].uc[2] = predictions[entry_oriented_index(l, 0, n*l.w*l.h + i, 6)]; // a3
                dets[count].uc[3] = predictions[entry_oriented_index(l, 0, n*l.w*l.h + i, 7)]; // a4
                dets[count].uc[4] = predictions[entry_oriented_index(l, 0, n*l.w*l.h + i, 8)]; // r

                for (j = 0; j < l.classes; ++j) {
                    int class_index = entry_oriented_index(l, 0, n*l.w*l.h + i, 10 + j);
    ...
    correct_oriented_yolo_boxes(dets, count, w, h, netw, neth, relative, letter);
    return count;
}