...
orient_box_label *read_orient_boxes(char *filename, int *n)
{
    orient_box_label* boxes = (orient_box_label*)xcalloc(1, sizeof(orient_box_label));
    FILE *file = fopen(filename, "r");
    if (!file) {
        printf("Can't open label file. (This can be normal only if you use MSCOCO): %s \n", filename);
        //file_error(filename);
    ...
    float x, y, h, w, a1, a2, a3, a4, r;
    int id;
    int count = 0;
    while(fscanf(file, "%d %f %f %f %f %f %f %f %f %f", &id, &x, &y, &w, &h, &a1, &a2, &a3, &a4, &r) == 10){
        boxes = (orient_box_label*)xrealloc(boxes, (count + 1) * sizeof(orient_box_label));
        boxes[count].id = id;
        boxes[count].x = x;
        boxes[count].y = y;
        boxes[count].h = h;
...
void randomize_orient_boxes(orient_box_label *b, int n)
{
    int i;
    for(i = 0; i < n; ++i){
        orient_box_label swap = b[i];
        int index = random_gen()%n;
        b[i] = b[index];
        b[index] = swap;
    }
...
void correct_orient_boxes(orient_box_label *boxes, int n, float dx, float dy, float sx, float sy, int flip)
{
    int i;
    for(i = 0; i < n; ++i){
        if(boxes[i].x == 0 && boxes[i].y == 0) {
        ...
        boxes[i].left   = boxes[i].left   * sx - dx;   // left = (left * ow - pleft) / swidth
        boxes[i].right  = boxes[i].right  * sx - dx;   // right = (right * ow - pleft) / swidth
        boxes[i].top    = boxes[i].top    * sy - dy;   // top = (top * oh - ptop) / sheight
        boxes[i].bottom = boxes[i].bottom * sy - dy;   // bot = (bot * oh - ptop) / sheight

//        boxes[i].a1 = boxes[i].a1;   //
//        boxes[i].a2 = boxes[i].a2;
//        boxes[i].a3 = boxes[i].a3;
//        boxes[i].a4 = boxes[i].a4;

        if(flip){
            float swap = boxes[i].left;
            boxes[i].left = 1. - boxes[i].right;
            boxes[i].right = 1. - swap;
            boxes[i].a1 = 1. - boxes[i].a1;
            boxes[i].a2 = 1. - boxes[i].a2;
            boxes[i].a3 = 1. - boxes[i].a3;
            boxes[i].a4 = 1. - boxes[i].a4;
        }
        ...
        boxes[i].a1 = constrain(0, 1, boxes[i].a1);
        boxes[i].a2 = constrain(0, 1, boxes[i].a2);
        boxes[i].a3 = constrain(0, 1, boxes[i].a3);
        boxes[i].a4 = constrain(0, 1, boxes[i].a4);
        boxes[i].r = constrain(0, 1, boxes[i].r);
    }
}
...
int fill_truth_orient_detection(const char *path, int num_boxes, float *truth, int classes, int flip, float dx, float dy, float sx, float sy,
                         int net_w, int net_h)
{
    char labelpath[4096];
    replace_image_to_label(path, labelpath);
    ...
    int count = 0;
    int i;
    orient_box_label *boxes = read_orient_boxes(labelpath, &count);
    int min_w_h = 0;
    float lowest_w = 1.F / net_w;
    float lowest_h = 1.F / net_h;
    randomize_orient_boxes(boxes, count);
    correct_orient_boxes(boxes, count, dx, dy, sx, sy, flip);
    if (count > num_boxes) count = num_boxes;
...
void blend_truth_orient_mosaic(float *new_truth, int boxes, float *old_truth, int w, int h, float cut_x, float cut_y, int i_mixup,
                        int left_shift, int right_shift, int top_shift, int bot_shift, int fix_bound)
{
    const int t_size = 4 + 5 + 1;
    int count_new_truth = 0;
    int t;
    for (t = 0; t < boxes; ++t) {
        float x = new_truth[t*(4 + 5 + 1)];
        if (!x) break;
        count_new_truth++;
    }
    int new_t = count_new_truth;
    ...
    float jitter, float resize, float hue, float saturation, float exposure, int mini_batch, int track, int augment_speed, int letter_box, int show_imgs)
{
    ...
    for (i_mixup = 0; i_mixup <= use_mixup; i_mixup++) {
        ...
        for (i = 0; i < n; ++i) {
            ...
            int count = 0, t = 0;
            box_label *gt_boxes = read_boxes(labelpath, &count);
            float min_left = 1, max_right = 0, min_top = 1, max_bot = 0;
            for (t = 0; t < count; ++t) {
                min_left = min_left > gt_boxes[t].left ? gt_boxes[t].left : min_left;
                max_right = max_right < gt_boxes[t].right ? gt_boxes[t].right : max_right;
                min_top = min_top > gt_boxes[t].top ? gt_boxes[t].top : min_top;
                max_bot = max_bot < gt_boxes[t].bottom ? gt_boxes[t].bottom : max_bot;
            }
            max_right = 1 - max_right;
            float min_bw = min_left > max_right ? max_right : min_left;
            max_bot = 1 - max_bot;
            float min_bh = min_top > max_bot ? max_bot : min_top;
            float min_bwh = min_bw > min_bh ? min_bh : min_bw;
            jitter = gt_jitter > min_bwh ? min_bwh : gt_jitter;
            ...
            */
            ...
            image ai = image_data_augmentation(src, w, h, pleft, ptop, swidth, sheight, flip, dhue, dsat, dexp,
                gaussian_noise, blur, boxes, truth, 0);
...
data load_data_orient_detection(int n, char **paths, int m, int w, int h, int c, int boxes, int classes, int use_flip, int use_gaussian_noise, int use_blur, int use_mixup,
                         float jitter, float resize, float hue, float saturation, float exposure, int mini_batch, int track, int augment_speed, int letter_box, int show_imgs)
{
    float gt_jitter = jitter;
    const int random_index = random_gen();
    ...
    for (i_mixup = 0; i_mixup <= use_mixup; i_mixup++) {
        ...
        for (i = 0; i < n; ++i) {
            float *truth = (float*)xcalloc(10 * boxes, sizeof(float));
            const char *filename = random_paths[i];
            ...
            int count = 0, t = 0;
            orient_box_label *gt_boxes = read_orient_boxes(labelpath, &count);
            float min_left = 1, max_right = 0, min_top = 1, max_bot = 0;
            for (t = 0; t < count; ++t) {
                min_left = min_left > gt_boxes[t].left ? gt_boxes[t].left : min_left;
                max_right = max_right < gt_boxes[t].right ? gt_boxes[t].right : max_right;
            ...
            */
            ...
            int min_w_h = fill_truth_orient_detection(filename, boxes, truth, classes, flip, dx, dy, 1. / sx, 1. / sy, w, h);

            if ((min_w_h / 8) < blur && blur > 1) blur = min_w_h / 8;   // disable blur if one of the objects is too small

            image ai = image_data_augmentation(src, w, h, pleft, ptop, swidth, sheight, flip, dhue, dsat, dexp,
                                               gaussian_noise, blur, boxes, truth, 1);

            if (use_mixup == 0) {
                d.X.vals[i] = ai.data;
                memcpy(d.y.vals[i], truth, 10 * boxes * sizeof(float));
            }
            else if (use_mixup == 1) {
                if (i_mixup == 0) {
                    d.X.vals[i] = ai.data;
                ...
                // start points in resized image (eg: 608 X 608)
                const int left_shift = min_val_cmp(cut_x[i], max_val_cmp(0, (-pleft*w / ow)));   // left_shift = pleft > 0 ? 0 : min(cut_x, -pleft*w / ow)
                const int top_shift = min_val_cmp(cut_y[i], max_val_cmp(0, (-ptop*h / oh)));     //

                const int right_shift = min_val_cmp((w - cut_x[i]), max_val_cmp(0, (-pright*w / ow)));   // right_shift = pright > 0 ? 0 : min(w - cut_x, -pright*w / ow)
                const int bot_shift = min_val_cmp(h - cut_y[i], max_val_cmp(0, (-pbot*h / oh)));

                //printf("\n pleft = %d, pright = %d, ptop = %d, pbot = %d, ow = %d, oh = %d ", pleft, pright, ptop, pbot, ow, oh);
                //printf("\n left_shift = %d, top_shift = %d, right_shift = %d, bot_shift = %d, cut_x = %d, cut_y = %d \n \n ", left_shift, top_shift, right_shift, bot_shift, cut_x[i], cut_y[i]);

                // cut and merge 4 images in one
                int k, x, y;
                for (k = 0; k < c; ++k) {      // channel
                    for (y = 0; y < h; ++y) {  // rows
                        int j = y*w + k*w*h;   // the start ptr of every cols
                        if (i_mixup == 0 && y < cut_y[i]) {
                            // ai (right, bot) ==> d.X.val (left, top),  size: (cut_x, cut_y)
                            int j_src = (w - cut_x[i] - right_shift) + (y + h - cut_y[i] - bot_shift)*w + k*w*h;
                            memcpy(&d.X.vals[i][j + 0], &ai.data[j_src], cut_x[i] * sizeof(float));
                        }
                        if (i_mixup == 1 && y < cut_y[i]) {
                            // ai (left, bot) ==> d.X.val (right, top), size: (w - cut_x, cut_y)
                            int j_src = left_shift + (y + h - cut_y[i] - bot_shift)*w + k*w*h;
                            memcpy(&d.X.vals[i][j + cut_x[i]], &ai.data[j_src], (w-cut_x[i]) * sizeof(float));
                        }
                        if (i_mixup == 2 && y >= cut_y[i]) {
                            // ai (right, top) ==> d.X.val (left, bot), size: (cut_x, h - cut_y)
                            int j_src = (w - cut_x[i] - right_shift) + (top_shift + y - cut_y[i])*w + k*w*h;
                            memcpy(&d.X.vals[i][j + 0], &ai.data[j_src], cut_x[i] * sizeof(float));
                        }
                        if (i_mixup == 3 && y >= cut_y[i]) {
                            // ai (left, top) ==> d.X.val (right, bot), size: (w - cut_x, h - cut_y)
                            int j_src = left_shift + (top_shift + y - cut_y[i])*w + k*w*h;
                            memcpy(&d.X.vals[i][j + cut_x[i]], &ai.data[j_src], (w - cut_x[i]) * sizeof(float));
                        }
                    }
                ...
                blend_truth_orient_mosaic(d.y.vals[i], boxes, truth, w, h, cut_x[i], cut_y[i], i_mixup, left_shift, right_shift, top_shift, bot_shift, 0);
            ...
            if (show_imgs && i_mixup == use_mixup)   // delete i_mixup
            {
                ...
                int t;
                for (t = 0; t < boxes; ++t) {
                    box b = float_to_box_stride(d.y.vals[i] + t*(4 + 5 + 1), 1);
                    if (!b.x) break;
                    int left = (b.x - b.w / 2.)*ai.w;
                    int right = (b.x + b.w / 2.)*ai.w;
                    int top = (b.y - b.h / 2.)*ai.h;
                    ...
                    orient_box ob = float_to_orient_box_stride(d.y.vals[i] + t*(4 + 5 + 1) + 4, 1);
                    int a1 = ob.a1 * b.w * ai.w;
                    int a2 = ob.a2 * b.h * ai.h;
                    int a3 = ob.a3 * b.w * ai.w;
                    int a4 = ob.a4 * b.h * ai.h;
                    tmp_ai = draw_orient_box(tmp_ai, left, right, top, bot, a1, a2, a3, a4);
                }
...
void *load_thread(void *ptr)
{
    ...
    } else if (a.type == ORIENT_DATA){
        *a.d = load_data_orient_detection(a.n, a.paths, a.m, a.w, a.h, a.c, a.num_boxes, a.classes, a.flip, a.gaussian_noise, a.blur, a.mixup, a.jitter, a.resize,
